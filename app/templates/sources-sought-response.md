# Sources Sought Response — {{LEGAL_NAME}}

## 1. Company identification

- Legal name, UEI, CAGE, SAM active status
- Small business and certification summary
- Primary and secondary NAICS codes

## 2. Capability statement

- Notice: {{NOTICE_ID}} — {{OPPORTUNITY_TITLE}}
- Agency: {{AGENCY_NAME}}
- Company: {{LEGAL_NAME}} · UEI {{UEI}} · CAGE {{CAGE}}
- NAICS: {{PRIMARY_NAICS}} (+ secondary as applicable)
- Set-aside: {{SET_ASIDE}}
- Response deadline: {{RESPONSE_DEADLINE}}
- Summary of relevant experience for this sources sought
- Core competencies aligned to scope
- Differentiators and win themes

## 3. Technical approach overview

- High-level methodology for performing the work
- Staffing and management approach
- Compliance and quality framework

## 4. Past performance

- Similar contracts (agency, value, scope, results)
- Healthcare / research / program management parallels

## 5. Socioeconomic / size status

- Small business representation
- Teaming interest if applicable

## 6. Point of contact

- {{CONTACT_EMAIL}} · {{PHONE}} · {{WEBSITE}}
